...
3. Confirm the Firefox add-on ID in `firefox-extension/manifest.json`
   - Current Gecko ID: `google-ai-scraper@balakumardev.com`
...
# Lint
web-ext lint --source-dir firefox-extension

# Build unsigned XPI
web-ext build --source-dir firefox-extension --artifacts-dir web-ext-artifacts

# Sign + submit listed add-on
web-ext sign \
  ...
  --channel listed \
  --amo-metadata=firefox-store-metadata.json
```

### Listing Content

- Store copy: `firefox-store-listing.md`
- AMO metadata for first listed submission: `firefox-store-metadata.json`
- Firefox promotional screenshots: `firefox-store-assets/` (generated by `firefox-store-assets/generate.py`)

### CI Auto-Publish

The repo includes `.github/workflows/publish-firefox-extension.yml`.

Add these GitHub secrets:

```bash
gh secret set AMO_JWT_ISSUER --body "your-jwt-issuer"
gh secret set AMO_JWT_SECRET --body "your-jwt-secret"
```

After that, pushes touching `firefox-extension/`, `firefox-store-metadata.json`, `firefox-store-listing.md`, or `scripts/generate_extension_icons.py` will lint, build, sign, and submit the Firefox add-on.

### Manual Update (alternative)

1. Bump `version` in `firefox-extension/manifest.json`
2. Run `web-ext lint` and `web-ext build`
3. Run `web-ext sign` with the metadata file
4. Verify the signed XPI in `web-ext-artifacts/`
...
## All Firefox Listing Text

Firefox listing copy is pre-written in:
**`firefox-store-listing.md`** — copy/paste into AMO as needed.

Firefox metadata for the `web-ext sign --amo-metadata` flow lives in:
**`firefox-store-metadata.json`**

Firefox promotional images can be generated from:
**`firefox-store-assets/`**
